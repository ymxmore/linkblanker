...
'use strict'; function Event(a, b, c) {
...
}let a=Event.prototype; a.preventDefault=function() {
...
'use strict'; function EventDispatcher() {
this._listeners=null, this._captureListeners=null;
}let a=EventDispatcher.prototype; EventDispatcher.initialize=function(b) {
...
if('string'==typeof a) {
let b=this._listeners; if(!b||!b[a])return!1; a=new createjs.Event(a);
}else a.target&&a.clone&&(a=a.clone()); try{
a.target=this;
}catch(c) {}if(a.bubbles&&this.parent) {
for(var d=this, e=[d]; d.parent;)e.push(d=d.parent); let f, g=e.length; for(f=g-1; f>=0&&!a.propagationStopped; f--)e[f]._dispatchEvent(a, 1+(0==f)); for(f=1; g>f&&!a.propagationStopped; f++)e[f]._dispatchEvent(a, 3);
...
return!Ticker._inited&&Ticker.init(), Ticker._addEventListener(...arguments);
...
if(Ticker._raf) {
let a=window.cancelAnimationFrame||window.webkitCancelAnimationFrame||window.mozCancelAnimationFrame||window.oCancelAnimationFrame||window.msCancelAnimationFrame; a&&a(Ticker._timerId);
...
let b=0, c=Ticker._tickTimes; if(!c||c.length<1)return-1; a=Math.min(c.length, a||0|Ticker.getFPS()); for(let d=0; a>d; d++)b+=c[d]; return b/a;
}, Ticker.getMeasuredFPS=function(a) {
let b=Ticker._times; return!b||b.length<2?-1:(a=Math.min(b.length-1, a||0|Ticker.getFPS()), 1e3/((b[0]-b[a])/a));
...
let a=Ticker.timingMode||Ticker.useRAF&&Ticker.RAF_SYNCHED; if(a==Ticker.RAF_SYNCHED||a==Ticker.RAF) {
let b=window.requestAnimationFrame||window.webkitRequestAnimationFrame||window.mozRequestAnimationFrame||window.oRequestAnimationFrame||window.msRequestAnimationFrame; if(b)return Ticker._timerId=b(a==Ticker.RAF?Ticker._handleRAF:Ticker._handleSynch), void(Ticker._raf=!0);
}Ticker._raf=!1, Ticker._timerId=setTimeout(Ticker._handleTimeout, Ticker._interval);
}
}, Ticker._tick=function() {
let a=Ticker.paused, b=Ticker._getTime(), c=b-Ticker._lastTime; if(Ticker._lastTime=b, Ticker._ticks++, a&&(Ticker._pausedTicks++, Ticker._pausedTime+=c), Ticker.hasEventListener('tick')) {
let d=new createjs.Event('tick'), e=Ticker.maxDelta; d.delta=e&&c>e?e:c, d.paused=a, d.time=b, d.runTime=b-Ticker._pausedTime, Ticker.dispatchEvent(d);
}for(Ticker._tickTimes.unshift(Ticker._getTime()-b); Ticker._tickTimes.length>100;)Ticker._tickTimes.pop(); for(Ticker._times.unshift(b); Ticker._times.length>100;)Ticker._times.pop();
}; let b=window.performance&&(performance.now||performance.mozNow||performance.msNow||performance.oNow||performance.webkitNow); Ticker._getTime=function() {
return(b&&b.call(performance)||(new Date).getTime())-Ticker._startTime;
}, createjs.Ticker=Ticker;
}(), this.createjs=this.createjs||{}, function() {
'use strict'; function Tween(a, b, c) {
...
}let a=createjs.extend(Tween, createjs.EventDispatcher); Tween.NONE=0, Tween.LOOP=1, Tween.REVERSE=2, Tween.IGNORE={}, Tween._tweens=[], Tween._plugins={}, Tween.get=function(a, b, c, d) {
return d&&Tween.removeTweens(a), new Tween(a, b, c);
}, Tween.tick=function(a, b) {
for(let c=Tween._tweens.slice(), d=c.length-1; d>=0; d--) {
let e=c[d]; b&&!e.ignoreGlobalPause||e._paused||e.tick(e._useTicks?1:a);
}
}, Tween.handleEvent=function(a) {
'tick'==a.type&&this.tick(a.delta, a.paused);
}, Tween.removeTweens=function(a) {
if(a.tweenjs_count) {
for(let b=Tween._tweens, c=b.length-1; c>=0; c--) {
let d=b[c]; d._target==a&&(d._paused=!0, b.splice(c, 1));
}a.tweenjs_count=0;
}
}, Tween.removeAllTweens=function() {
for(var a=Tween._tweens, b=0, c=a.length; c>b; b++) {
let d=a[b]; d._paused=!0, d.target&&(d.target.tweenjs_count=0);
}a.length=0;
...
let c=a.priority; null==c&&(a.priority=c=0); for(let d=0, e=b.length, f=Tween._plugins; e>d; d++) {
let g=b[d]; if(f[g]) {
for(var h=f[g], i=0, j=h.length; j>i&&!(c<h[i].priority); i++);f[g].splice(i, 0, a);
}else f[g]=[a];
}
}, Tween._register=function(a, b) {
let c=a._target, d=Tween._tweens; if(b&&!a._registered)c&&(c.tweenjs_count=c.tweenjs_count?c.tweenjs_count+1:1), d.push(a), !Tween._inited&&createjs.Ticker&&(createjs.Ticker.addEventListener('tick', Tween), Tween._inited=!0); else if(!b&&a._registered) {
c&&c.tweenjs_count--; for(let e=d.length; e--;)if(d[e]==a) {
d.splice(e, 1); break;
}
}a._registered=b;
}, a.wait=function(a, b) {
if(null==a||0>=a)return this; let c=this._cloneProps(this._curQueueProps); return this._addStep({d: a, p0: c, e: this._linearEase, p1: c, v: b});
}, a.to=function(a, b, c) {
...
0>a&&(a=0), null==b&&(b=1); let c=a, d=!1; if(c>=this.duration&&(this.loop?c%=this.duration:(c=this.duration, d=!0)), c==this._prevPos)return d; let e=this._prevPos; if(this.position=this._prevPos=c, this._prevPosition=a, this._target)if(d)this._updateTargetProps(null, 1); else if(this._steps.length>0) {
for(var f=0, g=this._steps.length; g>f&&!(this._steps[f].t>c); f++);let h=this._steps[f-1]; this._updateTargetProps(h, (this._stepPosition=c-h.t)/h.d);
...
let c, d, e, f, g, h; if(a||1!=b) {
if(this.passive=!!a.v, this.passive)return; a.e&&(b=a.e(b, 0, 1, 1)), c=a.p0, d=a.p1;
}else this.passive=!1, c=d=this._curQueueProps; for(let i in this._initQueueProps) {
null==(f=c[i])&&(c[i]=f=this._initQueueProps[i]), null==(g=d[i])&&(d[i]=g=f), e=f==g||0==b||1==b||'number'!=typeof f?1==b?g:f:f+(g-f)*b; let j=!1; if(h=Tween._plugins[i])for(let k=0, l=h.length; l>k; k++) {
let m=h[k].tween(this, i, e, c, d, b, !!a&&c==d, !a); m==Tween.IGNORE?j=!0:e=m;
}j||(this._target[i]=e);
}
}, a._runActions=function(a, b, c) {
let d=a, e=b, f=-1, g=this._actions.length, h=1; for(a>b&&(d=b, e=a, f=g, g=h=-1); (f+=h)!=g;) {
let i=this._actions[f], j=i.t; (j==e||j>d&&e>j||c&&j==a)&&i.f.apply(i.o, i.p);
}
}, a._appendQueueProps=function(a) {
let b, c, d, e, f; for(var g in a)if(void 0===this._initQueueProps[g]) {
if(c=this._target[g], b=Tween._plugins[g])for(d=0, e=b.length; e>d; d++)c=b[d].init(this, g, c); this._initQueueProps[g]=this._curQueueProps[g]=void 0===c?null:c;
}else c=this._curQueueProps[g]; for(var g in a) {
if(c=this._curQueueProps[g], b=Tween._plugins[g])for(f=f||{}, d=0, e=b.length; e>d; d++)b[d].step&&b[d].step(this, g, c, a[g], f); this._curQueueProps[g]=a[g];
}return f&&this._appendQueueProps(f), this._curQueueProps;
}, a._cloneProps=function(a) {
let b={}; for(let c in a)b[c]=a[c]; return b;
...
'use strict'; function Timeline(a, b, c) {
...
}let a=createjs.extend(Timeline, createjs.EventDispatcher); a.addTween=function(a) {
let b=arguments.length; if(b>1) {
for(let c=0; b>c; c++)this.addTween(arguments[c]); return arguments[0];
...
this._labels[a]=b; let c=this._labelList; if(c) {
for(var d=0, e=c.length; e>d&&!(b<c[d].position); d++);c.splice(d, 0, {label: a, position: b});
}
}, a.setLabels=function(a) {
this._labels=a?a:{};
}, a.getLabels=function() {
let a=this._labelList; if(!a) {
a=this._labelList=[]; let b=this._labels; for(let c in b)a.push({label: c, position: b[c]}); a.sort(function(a, b) {
return a.position-b.position;
});
}return a;
}, a.getCurrentLabel=function() {
let a=this.getLabels(), b=this.position, c=a.length; if(c) {
...
let c=this._calcPosition(a), d=!this.loop&&a>=this.duration; if(c==this._prevPos)return d; this._prevPosition=a, this.position=this._prevPos=c; for(let e=0, f=this._tweens.length; f>e; e++)if(this._tweens[e].setPosition(c, b), c!=this._prevPos)return!1; return d&&this.setPaused(!0), this.dispatchEvent('change'), d;
}, a.setPaused=function(a) {
this._paused=!!a, createjs.Tween._register(this, !a);
}, a.updateDuration=function() {
this.duration=0; for(let a=0, b=this._tweens.length; b>a; a++) {
let c=this._tweens[a]; c.duration>this.duration&&(this.duration=c.duration);
}
}, a.tick=function(a) {
this.setPosition(this._prevPosition+a);
}, a.resolve=function(a) {
let b=Number(a); return isNaN(b)&&(b=this._labels[a]), b;
}, a.toString=function() {
return'[Timeline]';
}, a.clone=function() {
throw'Timeline can not be cloned.';
}, a._goto=function(a) {
let b=this.resolve(a); null!=b&&this.setPosition(b);
}, a._calcPosition=function(a) {
return 0>a?0:a<this.duration?a:this.loop?a%this.duration:this.duration;
}, createjs.Timeline=createjs.promote(Timeline, 'EventDispatcher');
}(), this.createjs=this.createjs||{}, function() {
'use strict'; function Ease() {
...
let c=2*Math.PI; return function(d) {
if(0==d||1==d)return d; let e=b/c*Math.asin(1/a); return-(a*Math.pow(2, 10*(d-=1))*Math.sin((d-e)*c/b));
};
}, Ease.elasticIn=Ease.getElasticIn(1, .3), Ease.getElasticOut=function(a, b) {
let c=2*Math.PI; return function(d) {
if(0==d||1==d)return d; let e=b/c*Math.asin(1/a); return a*Math.pow(2, -10*d)*Math.sin((d-e)*c/b)+1;
};
}, Ease.elasticOut=Ease.getElasticOut(1, .3), Ease.getElasticInOut=function(a, b) {
let c=2*Math.PI; return function(d) {
let e=b/c*Math.asin(1/a); return(d*=2)<1?-.5*a*Math.pow(2, 10*(d-=1))*Math.sin((d-e)*c/b):a*Math.pow(2, -10*(d-=1))*Math.sin((d-e)*c/b)*.5+1;
};
}, Ease.elasticInOut=Ease.getElasticInOut(1, .3*1.5), createjs.Ease=Ease;
}(), this.createjs=this.createjs||{}, function() {
'use strict'; function MotionGuidePlugin() {
...
let d=a.target; return d.hasOwnProperty('x')||(d.x=0), d.hasOwnProperty('y')||(d.y=0), d.hasOwnProperty('rotation')||(d.rotation=0), 'rotation'==b&&(a.__needsRot=!0), 'guide'==b?null:c;
}, MotionGuidePlugin.step=function(a, b, c, d, e) {
if('rotation'==b&&(a.__rotGlobalS=c, a.__rotGlobalE=d, MotionGuidePlugin.testRotData(a, e)), 'guide'!=b)return d; let f, g=d; g.hasOwnProperty('path')||(g.path=[]); let h=g.path; if(g.hasOwnProperty('end')||(g.end=1), g.hasOwnProperty('start')||(g.start=c&&c.hasOwnProperty('end')&&c.path===h?c.end:0), g.hasOwnProperty('_segments')&&g._length)return d; let i=h.length, j=10; if(!(i>=6&&(i-2)%4==0))throw'invalid \'path\' data, please see documentation for valid paths'; g._segments=[], g._length=0; for(let k=2; i>k; k+=4) {
for(var l, m, n=h[k-2], o=h[k-1], p=h[k+0], q=h[k+1], r=h[k+2], s=h[k+3], t=n, u=o, v=0, w=[], x=1; j>=x; x++) {
let y=x/j, z=1-y; l=z*z*n+2*z*y*p+y*y*r, m=z*z*o+2*z*y*q+y*y*s, v+=w[w.push(Math.sqrt((f=l-t)*f+(f=m-u)*f))-1], t=l, u=m;
}g._segments.push(v), g._segments.push(w), g._length+=v;
}f=g.orient, g.orient=!0; let A={}; return MotionGuidePlugin.calc(g, g.start, A), a.__rotPathS=Number(A.rotation.toFixed(5)), MotionGuidePlugin.calc(g, g.end, A), a.__rotPathE=Number(A.rotation.toFixed(5)), g.orient=!1, MotionGuidePlugin.calc(g, g.end, e), g.orient=f, g.orient?(a.__guideData=g, MotionGuidePlugin.testRotData(a, e), d):d;
...
let c=a.__guideData, d=a.__rotGlobalE-a.__rotGlobalS, e=a.__rotPathE-a.__rotPathS, f=d-e; if('auto'==c.orient)f>180?f-=360:-180>f&&(f+=360); else if('cw'==c.orient) {
...
let h=e.guide; if(void 0==h||h===d.guide)return c; if(h.lastRatio!=f) {
let i=(h.end-h.start)*(g?h.end:f)+h.start; switch(MotionGuidePlugin.calc(h, i, a.target), h.orient) {
case'cw':case'ccw':case'auto':a.target.rotation+=h.rotOffS+h.rotDelta*f; break; case'fixed':default:a.target.rotation+=h.rotOffS;
}h.lastRatio=f;
}return'rotation'!=b||h.orient&&'false'!=h.orient?a.target[b]:c;
}, MotionGuidePlugin.calc=function(a, b, c) {
void 0==a._segments&&MotionGuidePlugin.validate(a), void 0==c&&(c={x: 0, y: 0, rotation: 0}); for(var d=a._segments, e=a.path, f=a._length*b, g=d.length-2, h=0; f>d[h]&&g>h;)f-=d[h], h+=2; let i=d[h+1], j=0; for(g=i.length-1; f>i[j]&&g>j;)f-=i[j], j++; let k=j/++g+f/(g*i[j]); h=2*h+2; let l=1-k; return c.x=l*l*e[h-2]+2*l*k*e[h+0]+k*k*e[h+2], c.y=l*l*e[h-1]+2*l*k*e[h+1]+k*k*e[h+3], a.orient&&(c.rotation=57.2957795*Math.atan2((e[h+1]-e[h-1])*l+(e[h+3]-e[h+1])*k, (e[h+0]-e[h-2])*l+(e[h+2]-e[h+0])*k)), c;
}, createjs.MotionGuidePlugin=MotionGuidePlugin;
}(), this.createjs=this.createjs||{}, function() {
'use strict'; let a=createjs.TweenJS=createjs.TweenJS||{}; a.version='NEXT', a.buildDate='Wed, 27 May 2015 18:12:44 GMT';
}();
